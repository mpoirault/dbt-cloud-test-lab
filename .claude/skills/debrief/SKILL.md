---
name: debrief
description: Learning close-out for a finished task. Use at the end of
  a task, before the commit proposal, to walk the user through the
  diff. A guided opener, a concept walk, a teach-back with a
...
bump), say so in one line and continue to the `commit` skill.
...
- If the user approves and a knowledge-base skill is available (here:
  `notion`), save the note with that skill. If no knowledge-base skill
  exists, do not save the note anywhere.
